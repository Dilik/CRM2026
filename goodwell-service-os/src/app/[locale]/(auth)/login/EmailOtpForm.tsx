...
export function EmailOtpForm({ email, onBack }: { email: string; onBack: () => void }) {
  ...
  async function onResend() {
    ...
    try {
      ...
      if (res.error) toast.error(t("login.error.emailFailed"));
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div className="text-center">
        ...
        <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1">{t("otp.subtitleEmail")}</p>
        <p className="text-sm font-mono mt-2 truncate">{email}</p>
      </div>
